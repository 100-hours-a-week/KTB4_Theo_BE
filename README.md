...
이번 프로젝트를 진행하며 백엔드 개발은 단순히 API를 구현하는 것이 아니라, 동시 요청이나 트랜잭션 실패, 외부 저장소 오류 같은 상황에서도 데이터 정합성과 서비스 안정성을 지키는 일이라는 것을 배웠습니다.

특히 SSE 알림, 이미지 저장, 동시성 문제를 직접 다루면서 정상 동작뿐 아니라 실패 상황과 예외 흐름까지 체계적으로 함께 설계해야 한다는 점을 경험했습니다. 또한 k6와 Actuator를 활용해 Polling과 SSE 의 구조적 차이를 비교하며, 기술은 장점만 보는 것이 아니라 새롭게 발생하는 비용적 trade off 관점까지 함께 고려해야 한다는 점도 배웠습니다.

이번 프로젝트를 통해 특정 기술을 사용하는 것보다 왜 필요한지, 어떤 문제가 생길 수 있는지, 실제로 개선되었는지를 측정하고 검증하는 과정이 더 중요하다는 것을 느꼈습니다.
